...
class LandingPageView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            'age_group_labels': json.dumps(list(age_group_summary.keys())), 'age_group_data': json.dumps([d['total'] for d in age_group_summary.values()]),
            'birth_mode_labels': json.dumps([i['birth_mode'] for i in birth_mode_summary]), 'birth_mode_data': json.dumps([i['total'] for i in birth_mode_summary]),
        })
        return context
...
def export_full_report_excel(request):
    ...
    headers = ['Timestamp', 'Report Date', 'Time Slot', 'Time of Birth', 'District', 'Local Municipality', 'Facility', 'Facility Type', 'Mother Full Name', 'Mother D.O.B.', 'Gravidity', 'Parity', 'Birth Mode', 'Born Before Arrival', 'Baby Number', 'Baby Gender', 'Baby Weight (grams)']
    header_font = Font(bold=True, color="FFFFFF")
    header_fill = openpyxl.styles.PatternFill(start_color="4F81BD", fill_type="solid")
    for col_num, title in enumerate(headers, 1):
        cell = sheet.cell(row=1, column=col_num, value=title); cell.font = header_font; cell.fill = header_fill; cell.alignment = Alignment(horizontal='center')
    
    for delivery in queryset:
        common_data = [
            delivery.timestamp.strftime('%Y-%m-%d %H:%M'), delivery.report_date, delivery.time_slot,
            delivery.delivery_time.strftime('%H:%M') if delivery.delivery_time else '', delivery.district,
            delivery.local_municipality, delivery.facility, delivery.facility_type, delivery.mother_full_name,
            delivery.mother_dob.strftime('%Y-%m-%d') if delivery.mother_dob else '', delivery.gravidity,
            delivery.parity, delivery.birth_mode, 'Yes' if delivery.born_before_arrival else 'No'
        ]
...
class DashboardReportFilterView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['form'] = DashboardReportFilterForm()
        context['form_title'] = 'Generate Dashboard PDF Report'
        return context

class GenerateDashboardPDF(LoginRequiredMixin, UserPassesTestMixin, View):
    def test_func(self):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if context.get('selected_facility'): report_title = f"{context.get('selected_facility')} Report"
        elif context.get('selected_municipality'): report_title = f"{context.get('selected_municipality')} Report"
        elif context.get('selected_district'): report_title = f"{context.get('selected_district')} Report"
