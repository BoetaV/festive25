...
class UserFormMixin(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # This populates the municipality/facility dropdowns for validation and editing.
        if data:
            try:
            ...
            if profile.district:
                self.fields['local_municipality'].choices = [(m, m) for m in LOCATION_DATA['municipalities'].get(profile.district, [])]
            if profile.local_municipality:
                self.fields['facility'].choices = [(f, f) for f in LOCATION_DATA['facilities'].get(profile.local_municipality, [])]
        
        # --- Logic for Role-Based Permissions ---
        # This restricts the form fields based on the logged-in user's role.
...
class UserCreateForm(UserFormMixin, forms.ModelForm):
    # This form remains unchanged, it just inherits the new mixin logic.
...
class UserUpdateForm(UserFormMixin, forms.ModelForm):
    # This form also remains unchanged, inheriting the new mixin logic.
    first_name = forms.CharField(label="Name", min_length=3, required=True)
    last_name = forms.CharField(label="Surname", required=True)
    email = forms.EmailField(required=False)
    ...
    def save(self, commit=True):
        ...
        profile.persal_number = self.cleaned_data['persal_number']
        ...
        profile.local_municipality = self.cleaned_data.get('local_municipality')
